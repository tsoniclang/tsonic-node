...
2. **Review the three-file system** architecture (types/*.d.ts, types/*.metadata.json, types/Tsonic.StdLib.bindings.json)
...
3. **Bindings** (`types/Tsonic.StdLib.bindings.json`)
   - Maps JavaScript module names to CLR types
   - Single file for all modules
   - Example: `"fs"` → `"Tsonic.StdLib.fs"`
...
├── Tsonic.StdLib.bindings.json  # Single bindings file
└── index.d.ts                # Triple-slash references

tests/Tsonic.StdLib.Tests/   # xUnit tests
├── PathTests.cs
├── FsTests.cs
...
1. Create C# implementation in `src/Tsonic.StdLib/<module>.cs`
2. Copy or create `.d.ts` file in `types/<module>.d.ts`
3. Add binding to `types/Tsonic.StdLib.bindings.json`
4. Add reference in `types/index.d.ts`
5. Create tests in `tests/Tsonic.StdLib.Tests/<Module>Tests.cs`
