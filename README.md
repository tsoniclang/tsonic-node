...
Tsonic compiles TypeScript to C# so code can run on .NET. While the [Tsonic Runtime](https://github.com/tsoniclang/tsonic-runtime) provides JavaScript standard library functionality (console, Math, JSON, etc.), **nodejs** implements Node.js-inspired APIs like `fs`, `path`, and `events`.
...
- **C# Implementation** (`nodejs`) - A .NET library with Node.js-inspired APIs optimized for .NET
...
2. Use bindings to map `import "fs"` → `nodejs.fs`
