...
public class PathModule
{
    ...
    public string basename(string path, string? suffix = null) => Tsonic.StdLib.path.basename(path, suffix);

    /// <summary>Returns the directory name of a path.</summary>
    public string dirname(string path) => Tsonic.StdLib.path.dirname(path);

    /// <summary>Returns the extension of the path.</summary>
    public string extname(string path) => Tsonic.StdLib.path.extname(path);

    /// <summary>Joins all path segments together.</summary>
    public string join(params string[] paths) => Tsonic.StdLib.path.join(paths);

    /// <summary>Normalizes the given path.</summary>
    public string normalize(string path) => Tsonic.StdLib.path.normalize(path);

    /// <summary>Resolves a sequence of paths to an absolute path.</summary>
    public string resolve(params string[] paths) => Tsonic.StdLib.path.resolve(paths);

    /// <summary>Determines if path is an absolute path.</summary>
    public bool isAbsolute(string path) => Tsonic.StdLib.path.isAbsolute(path);

    /// <summary>Returns the relative path from 'from' to 'to'.</summary>
    public string relative(string from, string to) => Tsonic.StdLib.path.relative(from, to);

    /// <summary>Returns an object representing significant elements of the path.</summary>
    public ParsedPath parse(string path) => Tsonic.StdLib.path.parse(path);

    /// <summary>Returns a path string from an object.</summary>
    public string format(ParsedPath pathObject) => Tsonic.StdLib.path.format(pathObject);

    /// <summary>Determines if path matches the glob pattern.</summary>
    public bool matchesGlob(string path, string pattern) => Tsonic.StdLib.path.matchesGlob(path, pattern);

    /// <summary>Returns an equivalent namespace-prefixed path.</summary>
    public string toNamespacedPath(string path) => Tsonic.StdLib.path.toNamespacedPath(path);
